...
class TradingApp:
    ...
    def _setup_style(self) -> None:
        ...
        style.configure("Treeview.Heading", background=BG, foreground=MUTED, font=("Consolas", 9, "bold"))
        style.map("Treeview", background=[("selected", BLUE)], foreground=[("selected", BG)])
        style.configure("TFrame", background=BG)
        style.configure("TLabel", background=BG, foreground=TEXT)
        style.configure("TButton", background=PANEL, foreground=TEXT, bordercolor=BORDER, padding=6)
        style.map("TButton", background=[("active", BORDER)])
        style.configure("Start.TButton", background="#1a3a1a", foreground=GREEN, bordercolor=GREEN)
        style.map("Start.TButton", background=[("active", "#243a24")])
        style.configure("Stop.TButton", background="#3a1a1a", foreground=RED, bordercolor=RED)
        style.map("Stop.TButton", background=[("active", "#4a2a2a")])
        style.configure("TEntry", fieldbackground=PANEL, foreground=TEXT, insertcolor=TEXT, bordercolor=BORDER)
    ...
    def _build_header(self) -> None:
        ...
        tk.Label(hdr, text="Agent Trading Demo", font=("Consolas", 13, "bold"), bg=PANEL, fg=TEXT).grid(
        ...
        for col, (label, attr, color) in enumerate(
            ...
            frame = tk.Frame(hdr, bg=PANEL)
            frame.grid(row=0, column=col, padx=20)
            tk.Label(frame, text=label, font=("Consolas", 8), bg=PANEL, fg=MUTED).pack()
            lbl = tk.Label(frame, text="—", font=("Consolas", 12, "bold"), bg=PANEL, fg=color)
            lbl.pack()
            setattr(self, attr, lbl)

        self._lbl_countdown = tk.Label(hdr, text="", font=("Consolas", 10), bg=PANEL, fg=YELLOW)
    ...
    def _build_body(self) -> None:
        ...
        log_tab = tk.Frame(notebook, bg=PANEL)
        notebook.add(log_tab, text="  Trade Log  ")
    ...
    def _panel(self, parent, title: str, row: int, col: int) -> tk.Frame:
        ...
        tk.Label(outer, text=f"  {title}", font=("Consolas", 9, "bold"), bg=PANEL, fg=MUTED, anchor="w").grid(
    ...
    def _build_chart(self, parent: tk.Frame) -> None:
        tk.Label(parent, text="  Portfolio Value", font=("Consolas", 9, "bold"), bg=PANEL, fg=MUTED, anchor="w").pack(
            fill="x", pady=(6, 0)
        )
    ...
    def _build_holdings(self, parent: tk.Frame) -> None:
        ...
        for col in cols:
            self._tree_hold.heading(col, text=col)
            self._tree_hold.column(col, width=60, anchor="center")
        self._tree_hold.column("Symbol", width=55, anchor="center")
        scrollbar = ttk.Scrollbar(parent, orient="vertical", command=self._tree_hold.yview)
        self._tree_hold.configure(yscrollcommand=scrollbar.set)
    ...
    def _build_votes(self, parent: tk.Frame) -> None:
        ...
        for col in cols:
            self._tree_vote.heading(col, text=col)
            self._tree_vote.column(col, width=widths.get(col, 60), anchor="center")
        scrollbar = ttk.Scrollbar(parent, orient="vertical", command=self._tree_vote.yview)
        self._tree_vote.configure(yscrollcommand=scrollbar.set)
    ...
    def _build_log(self, parent: tk.Frame) -> None:
        self._log = tk.Text(parent, bg=BG, fg=TEXT, font=("Consolas", 9), insertbackground=TEXT, wrap="none", state="disabled", relief="flat")
    ...
    def _build_footer(self) -> None:
        ...
        self._btn_start = ttk.Button(footer, text="▶  Start", style="Start.TButton", command=self._start, width=12)
        self._btn_start.pack(side="left", padx=(14, 6))

        self._btn_stop = ttk.Button(footer, text="■  Stop", style="Stop.TButton", command=self._stop, width=12, state="disabled")
        self._btn_stop.pack(side="left", padx=6)

        tk.Label(footer, text="Cash $:", font=("Consolas", 9), bg=PANEL, fg=MUTED).pack(side="left", padx=(24, 2))
        self._ent_cash = tk.Entry(footer, width=7, bg=PANEL, fg=TEXT, insertbackground=TEXT, font=("Consolas", 9), relief="solid", bd=1)
        ...
        self._ent_agents = tk.Entry(footer, width=5, bg=PANEL, fg=TEXT, insertbackground=TEXT, font=("Consolas", 9), relief="solid", bd=1)
        ...
        self._ent_interval = tk.Entry(footer, width=5, bg=PANEL, fg=TEXT, insertbackground=TEXT, font=("Consolas", 9), relief="solid", bd=1)
        ...
        self._ent_cycles = tk.Entry(footer, width=5, bg=PANEL, fg=TEXT, insertbackground=TEXT, font=("Consolas", 9), relief="solid", bd=1)
    ...
    def _redraw_chart(self) -> None:
        ...
        if len(self._chart_times) > 1:
            ...
            ax.text(0.5, 0.5, "Waiting for first cycle…", transform=ax.transAxes, ha="center", va="center", color=MUTED, fontsize=10)
            ax.set_xticks([])
            ax.set_yticks([])
    ...
    def _load_history(self) -> None:
        ...
        if rows:
            for ts_str, _cycle, value, _cash, _pnl_pct in rows:
                try:
    ...
    def _engine_loop(self, cash: float, agent_count: int, interval: int, max_cycles: int, persisted_agents: list[dict] | None) -> None:
        market = MarketData()
    ...
    def _handle_cycle(self, result: CycleResult) -> None:
        if self._session_id is not None:
            self._db.save_snapshot(self._session_id, result.cycle, result.timestamp, result.portfolio_value, result.cash, self._initial_cash)
        ...
        learn_text = f"C {result.learning_summary.get('avg_confidence', 0.0):.2f} | E {result.learning_summary.get('avg_exploration', 0.0):.2f}"
        ...
        for trade in result.trades:
            self._append_trade_to_log(result.timestamp.isoformat(), result.cycle, trade.action, trade.symbol, trade.qty, trade.amount, trade.price, trade.vote_ratio)
    ...
    def _update_holdings(self, result: CycleResult) -> None:
        ...
        for symbol, holding in sorted(result.holdings.items(), key=lambda item: item[0]):
            current_price = result.prices.get(symbol, 0.0)
            value = holding.qty * current_price
            pnl_pct = (current_price / max(1e-9, holding.avg_price) - 1.0) * 100.0
            pnl_str = f"{'+' if pnl_pct >= 0 else ''}{pnl_pct:.1f}%"
            tag = "pos" if pnl_pct >= 0 else "neg"
            self._tree_hold.insert(
                "",
                "end",
                values=(symbol, f"{holding.qty:.3f}", f"{holding.avg_price:.2f}", f"{current_price:.2f}", f"{value:.2f}", pnl_str),
                tags=(tag,),
            )
    ...
    def _update_votes(self, result: CycleResult) -> None:
        ...
        for symbol, vote in sorted(result.vote_summary.items(), key=lambda item: -item[1]["buy_ratio"]):
            buy_pct = vote["buy_ratio"] * 100
            sell_pct = vote["sell_ratio"] * 100
            ...
            self._tree_vote.insert("", "end", values=(symbol, f"{buy_pct:.0f}%", f"{sell_pct:.0f}%", signal), tags=(tag,))

    def _log_cycle_header(self, result: CycleResult) -> None:
        ts = result.timestamp.strftime("%H:%M:%S")
        universe = ", ".join(result.universe) if result.universe else "—"
        regime = result.market_regime.replace("_", " ")
        leader = result.learning_summary.get("leader_personality", "balanced")
        self._log_write(f"\n[{ts}] Cycle {result.cycle}  |  {regime}  |  leader={leader}  |  {universe}\n", "cycle")

    def _append_trade_to_log(self, ts_str: str, cycle: int, action: str, symbol: str, qty: float, amount: float, price: float, vote_ratio: float, from_history: bool = False) -> None:
        try:
            dt = datetime.fromisoformat(ts_str)
        ...
        line = f"{prefix}{hms}  {action:<4}  {symbol:<5}  qty={qty:.4f}  ${amount:.2f}  @${price:.2f}  vote={vote_ratio:.0%}\n"
